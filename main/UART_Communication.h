...
void Recive_Data()
{
  ...
  if ( millis()-last_minute >=1800 and auto_receiving == true) //sau khi cho 1.8s ma ko nhan dc data moi thi xoa data cu
  {
    ...
    {
      data_software !="";    
      Serial.println("LOG Timeout connection. Reset data va chuan bi nhan data moi");
      info1=true;
      info2 = false; 
      info_receive = false;
    }    
  ...
  if (data_software == "AUT") // nhận và gửi phản hồi về data
  {
    if (info == false)
    {
      Serial.println("LOG Receiving AUTO data...");
      info = true;   
      info2 = false;   
      Serial.println("N"); // gửi N để next value trong chuỗi auto   
      info_receive = false;
      last_minute = millis(); //khởi động tính thời gian timeout connection 
    }
    auto_receiving = true;   
  ...
  if (last_index == true)
    {
      Serial.println("LOG Data full received. Checking data...");      
      excute_string = data_software;
      delay(200);
      Serial.println(data_software);
  ...
  {
    delay(100);
    Serial.println("LOG Nhan duoc tin hieu BEGIN. Gui NEXT len PC");
    delay(200);
    Serial.println("NEXT");
    data_software = "";
    delay(300);
  ...
  if (data_software == "DATA NG")
  {
    ...
    Serial.println("LOG Data NG. Retry... "); 
    Serial2.println(data_software); 
    data_software = "";   
    last_index = false;
    info = false;
  ...
  {    
    ...
    data_index = 0;
    data_software = "";
    Serial.println("LOG Data is ok. Pass to excute string: " + excute_string); 
  ...
  if (auto_receiving == false and data_software != "" )
  {
    ...
    {
    Serial.println("LOG Done Excute Manual: " + data_software);   
    data_software ="";
    auto_data_received = false;
    data_index =0;
    info = false;
...
void cut_and_excute()
{
  ...
  {
    Serial.println("LOG Bat dau vong lap");
    ...
    Serial.println("LOG Data trim : " + data_trim);
    ...
    Serial.println("LOG Send 1 len PC");
    Serial.println(1);
  }
  auto_data_received = false;
  excute_string ="";
  ...
  Serial.println("LOG Thuc hien xong chu");
}
#endif
