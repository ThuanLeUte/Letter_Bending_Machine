...
#define SS7_HOLES_PIN SS5

#define SS7_END_STROKE_FRONT_PIN SS7
#define SS6_MATERIAL_PIN SS8
...
void toi_cat_trong()
{
    ...
    while (digitalRead(SS7_END_STROKE_FRONT_PIN) == 0 and digitalRead(BUTTON_EMERGENCY_PIN) == 0 
      and (digitalRead(BUTTON_STOP_PIN) == 0) and count < 5000)
    {
      delay(1);
      count +=1;
    }
...
void lui_cat_trong()
{
  if (Appl_ButtonStopPress_xdu == false and Appl_EmergencyHold_xdu == false)
  {
    ...
    uint16_t count = 0;
    while (digitalRead(SS4_END_STROKE_BACK_PIN) == 0 and digitalRead(BUTTON_EMERGENCY_PIN) == 0 
...
//  long current_pos = 0;
  ...
  Serial2.println("LOG Dang move ve A");
  while ((digitalRead(SS1_MOVE_HOME_A_PIN) == 0) and (digitalRead(BUTTON_EMERGENCY_PIN) == 0)
  ...
  Serial2.println("LOG Home Move A speed :" + String(STEPPER_MOVE.speed()));
  STEPPER_MOVE.setCurrentPosition(0);
  Serial2.println("LOG Xong move A");
  hole_index = 0;
  du_truoc = 0;
}
void Home_Move_B()
{
  ...
  while ((digitalRead(SS2_MOVE_HOME_B_PIN) == 1) and (digitalRead(BUTTON_EMERGENCY_PIN) == 0) 
  ...
  STEPPER_MOVE.stop();
  Serial2.println("LOG Finished Home Move B");
}
...
void Cutter_Forward()
{
  ...
  if (Appl_EmergencyHold_xdu == false and Appl_EmergencyHold_xdu == false)
  {
    ...
    digitalWrite(SOL_SLIDE_FORWARD_PIN, LOW);
    // wait xy lanh move ra khoi end stroke
    while (digitalRead(SS7_END_STROKE_FRONT_PIN) == 0 and digitalRead(BUTTON_STOP_PIN) == 0
      and digitalRead(BUTTON_EMERGENCY_PIN) == 0 and count <= 6000) //đợi xxy lanh chạm sensor ngoài
    {  
      delay(1);
      count += 1;
    }
    ...
    Serial2.println("LOG xy lanh da move toi diem ngoai cung");    
   }
}
...
void Cutter_Backward()
{
  ...
  {
    uint16_t count = 0;
    Serial2.println("LOG Bat dau lui dao");
    digitalWrite(SOL_SLIDE_FORWARD_PIN, HIGH);
    digitalWrite(SOL_SLIDE_BACKWARD_PIN, LOW);
    
    while (digitalRead(SS4_END_STROKE_BACK_PIN) == 0 and count <=6000        
      and digitalRead(BUTTON_STOP_PIN) == 0 and digitalRead(BUTTON_EMERGENCY_PIN) == 0)  //đợi xy lanh quay về
    {
      delay(1);
      count += 1;
    }
    ...
    Serial2.println("LOG Xong lui dao");
  }
}
...
void Angle_Cut(int16_t Step_Remain)
{
  ...
  while (STEPPER_CUT.currentPosition() != Step_Remain and (digitalRead(BUTTON_STOP_PIN) == 0) 
    and (digitalRead(BUTTON_EMERGENCY_PIN) == 0) and Appl_ButtonStopPress_xdu == false)
  {
...
void Brushless_Run(int Speed)
{
  if (digitalRead(BUTTON_EMERGENCY_PIN) == 0 and (digitalRead(BUTTON_STOP_PIN) == 0))
...
void Home_Cutter()
{
  Serial2.println("LOG Bat dau home Xoay dao");
  
  STEPPER_CUT.setSpeed(4000);
  uint16_t current_pos = STEPPER_CUT.currentPosition();
  while (digitalRead(SS3_CUT_HOME_PIN) == 0 
    and STEPPER_CUT.currentPosition() - current_pos <21000)
  {
    STEPPER_CUT.runSpeed();
    if (digitalRead(BUTTON_STOP_PIN) == 1)
    {
  ...
  STEPPER_CUT.setCurrentPosition(0);
  delay(200);
  Serial2.println("LOG Xong home Xoay dao");
}
void Home_All()
{
  Serial2.println("LOG Bat dau Home All");
  delay(200);
  Serial2.println(6);
  ...
  Serial2.println("LOG Finished Backward");
  delay(200);
  Home_Cutter();
  if (Appl_EmergencyHold_xdu == true or Appl_ButtonStopPress_xdu == true)
  {
  ...
  Serial2.println("LOG Finished Home Cutter");
  Center_Stepper_Cutter();
  if (Appl_EmergencyHold_xdu == true or Appl_ButtonStopPress_xdu == true)
  {
    return;
  }
  Serial2.println("LOG Finished Center Cutter");
